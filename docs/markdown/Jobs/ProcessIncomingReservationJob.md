...
**Full Class Name:** `App\TravelClick\Jobs\InboundJobs\ProcessIncomingReservationJob`

**File:** `Jobs/InboundJobs/ProcessIncomingReservationJob.php`
...
- `$rawXml` (string): The raw XML message received from TravelClick
- `$hotelCode` (string): The hotel code associated with the reservation
- `$messageId` (string): The message ID assigned to this reservation request
...
```php
public function handle(App\TravelClick\Parsers\ReservationParser $parser, App\TravelClick\Services\SoapService $soapService, App\TravelClick\Builders\ReservationResponseXmlBuilder $xmlBuilder): void
```

**Parameters:**

- `$parser` (ReservationParser): The parser for processing the reservation XML
- `$soapService` (SoapService): The service for sending SOAP responses
- `$xmlBuilder` (ReservationResponseXmlBuilder): The builder for creating response XML
